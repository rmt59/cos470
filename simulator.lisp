; [[file:simulator.org::+begin_src lisp +n -i :tangle yes :comments link
...
; (export '(forward-sensor][Package setup:1]]
(unless (find-package "SIM")
  (defpackage "SIMULATOR"
    (:use "COMMON-LISP")
    (:nicknames "SIM"))
...
; (export '(forward-sensor][Package setup:2]]
#+:sbcl (declaim (sb-ext:muffle-conditions style-warning))
; Package setup:2 ends here

; [[file:simulator.org::+begin_src lisp :tangle yes :comments link
; (load "new-symbol")
; (use-package 'sym)
; (load "messages")
; (shadowing-import 'msg:msg)
...
; (export '(forward-sensor][Package setup:3]]
(load "new-symbol")
(use-package 'sym)
(load "messages")
(shadowing-import 'msg:msg)
...
; (export '(forward-sensor][Global variables:1]]
(defvar *directions* '(:north :south :east :west))
...
; (export '(forward-sensor][Classes:1]]
(defclass simulator ()
  (
   (world :initarg :world :initform nil)
   (time :initarg :time :initform 0)
...
; (export '(forward-sensor][Simulator methods:1]]
(defmethod clear ((self simulator))
  (with-slots (world) self
    (clear world)))
...
; (export '(forward-sensor][Simulator methods:2]]
(defmethod add-obstacle ((self simulator) (object object))
  (with-slots (world) self
    (add-object world object)))
...
; (export '(forward-sensor][Sensor methods:1]]
(defmethod calculate-percept ((self simulator) (object object))
  )
...
; [[file:simulator.org::front-bump rear-bump left-bump right-bump bump-sensor))
; #+end_src
...
; Here are the supplied =take-action= methods:][Effector (actuator) methods:1]]
...
; ** Example: =random-robot=][Example: =random-robot=:1]]
...
; Now restore warning settings:][Example: =random-robot=:2]]
